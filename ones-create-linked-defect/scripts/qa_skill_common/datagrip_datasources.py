...
密码来源（按优先级）:
    ...
    3. 技能内 scripts/config/databases.yaml 或 qa_skill_common/config/databases.yaml（模板，不含真实密码；仅作默认 user 等兜底）
...
def _skill_template_candidates():
    """技能内 databases.yaml 模板候选路径（按文档从技能根执行时 cwd 含 scripts/config）。"""
    cands = []
    env = os.environ.get("DATAGRIP_SKILL_CONFIG", "").strip()
    if env:
        cands.append(Path(env))
    cands.append(Path.cwd() / "scripts" / "config" / "databases.yaml")
    cands.append(Path(__file__).resolve().parent / "config" / "databases.yaml")
    return cands
...
def load_password_config():
    ...
    skill_cfg = {}
    for cand in _skill_template_candidates():
        skill_cfg = _read_yaml_file(cand)
        if skill_cfg:
            break
