...
3. **新建关联缺陷**（若清单里有未登记的缺陷）：
   - **优先 API 直连提交（混合模式）**：
     1. 若字段选项 uuid 未缓存，先用 `open_defect_form()` + `capture_field_options()`
        （网络捕获失败时改用 `capture_field_options_fiber()`）
     ...
     2. 之后全部走 `build_defect_fields()`（主工单取来源项目/功能模块/产品负责人/优先级，
        ...
        + `create_linked_defect()` 创建并关联，每条秒级完成。
...
- 下拉选项异步加载且 teleport 到 body：聚焦搜索输入框输入关键词后
  ...
  选项 uuid 从 React fiber 的 `memoizedProps.options` 拿
  （`capture_field_options()` / `scripts/bbt_ones_env_uuid_ui.py`），DOM 不暴露。
...
- `scripts/ones_helpers.py`：CDP 连接 + ONES 接口封装
  （search_user / get_task_info / get_transitions / send_comment /
   build_defect_fields / create_linked_defect）
  + 新建缺陷弹窗稳定交互
  （open_defect_form / set_select_option / set_desc / upload_evidence /
   submit_defect / capture_field_options / list_related_tasks / dedup_check）。
