...
#
# **阈值刻意留 ~8% 余量**（≈ 立阈值时的实际值 ÷ 0.92）。
# 预算是「防大幅反弹」的兜底，不是「不许新增」的硬闸：贴着实际值立阈值会逼出坏行为——
# 为了让闸门通过而删掉必需规则，或把多条约束合并成一句把语义稀释掉
# （check_skill_docs 明确只防「删条」、防不住「合并稀释」）。
# 超预算时的正确顺序：① 优化表达（去重复、去已在 baseline 详载的解释性文字）
# ② 确属必需则上调 BUDGETS 并在提交信息里写明理由 ③ **禁止**为过闸删除红线、
# 停顿边界、结束闸门等硬约束。
BUDGETS = {
    "必读·标准功能测": 13100,
    "常见·生成+执行": 23400,
...
def main(argv=None):
    ...
    if args.check:
        if not ok:
            print("\n超预算时的正确顺序：① 优化表达（去重复）② 确属必需再上调 BUDGETS 并写明理由"
                  " ③ 禁止为过闸删除红线/停顿边界/结束闸门等硬约束")
