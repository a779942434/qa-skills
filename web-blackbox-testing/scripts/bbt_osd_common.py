# -*- coding: utf-8 -*-
"""欧斯达 MES 黑盒测试公共工具箱：登录/导航/下拉/表单/表格/造数（幂等）。

用法：`from bbt_osd_common import ...`，一次登录跑完多个用例/造数步骤。
"""
import time
...
PD_URL = URL + "/master-data/product/product-message-definition/index"
CR_URL = URL + "/master-data/technology/process-route-definition/index"
OSD_URL = URL + "/plan/work-plan/order-cycle-definition-OSD/index"
ACCOUNT = "admin"
PASSWORD = "sygl123456"


def login_ousida(page):
    """Keycloak 登录 t-ousida。"""
...
def wait_button(page, text, timeout=15000):
    """轮询直到指定文本按钮可见。"""
    deadline = time.time() + timeout
    while time.time() < deadline:
        try:
