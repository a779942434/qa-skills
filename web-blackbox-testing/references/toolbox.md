...
- `scripts/session_helpers.py`：一次会话常驻浏览器助手。`start_persistent_session` / `connect_persistent_session` / `ensure_mes_session` / `restart_persistent_session` / `stop_persistent_session` 默认使用仓库外用户数据目录和 CDP 9222；`cdp_health` 同时校验 `/json/version` + `/json/list`，健康失败自动回收受管 PID 并重启。
