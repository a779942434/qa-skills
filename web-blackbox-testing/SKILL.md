...
3. **默认后台专用运行**：MES 自动化必须使用无头模式和专用独立 profile，不得连接用户日常 Chrome/Edge profile、现有标签页或 ONES CDP `9334`。专用 profile 可跨任务保留登录态，但同一时间只允许一个任务持有。只有用户明确要求“可见窗口/人工接管”时，才允许前台模式。
